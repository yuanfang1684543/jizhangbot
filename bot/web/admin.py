...
from bot.database.models import Operator, AllMembersFlag, GroupSetting, IndividualConfig, AdSchedule, AutoReply
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .badge { display: inline-block; padding: 2px 8px; border-radius: 10px; font-size: 12px; background: #e8f0fe; color: #1a73e8; }
    ...
    <div class="container">
        ...
        <div class="card">
            ...
            <form hx-post="/api/members" hx-target="#result">
                ...
                <div class="form-row">
                    <label>姓名</label>
                    <input type="text" name="full_name" placeholder="用户名">
                </div>
                <div class="form-row">
                    <label>权限</label>
                    <select name="role">
        ...
        <div class="card">
            <h2>⚙️ 功能开关</h2>
            <p style="color: #666; font-size: 13px;">通过群内命令控制各项功能开关</p>
            <div style="margin-top: 10px; font-size: 13px; color: #666;">
                • 开启/关闭代付模式 • 开启/关闭分类 • 开启/关闭分类缩展<br>
                • 开启/关闭实时汇率 • 设置/关闭定时日切
            </div>
        </div>
        ...
        <div class="card">
            ...
            <form hx-post="/api/ads" hx-target="#result">
                ...
                <div class="form-row">
                    <label>时间间隔</label>
                    <input type="text" name="cron_expression" placeholder="每30分钟" value="*/30 * * * *">
                </div>
        ...
        <div class="card">
            ...
            <form hx-post="/api/auto-replies" hx-target="#result">
                ...
                <div class="form-row">
                    <label>回复内容</label>
                    <textarea name="reply_content" rows="2" required></textarea>
                </div>
...
@admin_app.route("/api/group-settings", methods=["POST"])
...
@admin_app.route("/api/members", methods=["POST"])
...
@admin_app.route("/api/ads", methods=["POST"])
async def save_ad():
    data = request.form
    group_id = int(data.get("group_id", 0))
...
@admin_app.route("/api/auto-replies", methods=["POST"])
